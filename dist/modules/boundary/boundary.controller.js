...
const boundary_service_1 = require("./boundary.service");
// ──────────────────────────────────────────────────────────────────────────────
// Boundary Controller
//
...
class BoundaryController {
    ...
    async assignBoundary(req, res) {
        ...
        const boundary = await boundary_service_1.boundaryService.assignBoundary(targetUserId, policyId, req.user.orgId, req.user.id);
        res.status(200).json({
            success: true,
    ...
    async getBoundary(req, res) {
        const targetUserId = req.params.id;
        const boundary = await boundary_service_1.boundaryService.getBoundary(targetUserId, req.user.orgId);
        res.status(200).json({
            success: true,
    ...
    async removeBoundary(req, res) {
        const targetUserId = req.params.id;
        await boundary_service_1.boundaryService.removeBoundary(targetUserId, req.user.orgId, req.user.id);
        res.status(200).json({
            success: true,
