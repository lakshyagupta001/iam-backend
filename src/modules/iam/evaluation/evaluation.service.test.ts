...
import { permissionService } from './evaluation.service';
import { permissionRepository } from './evaluation.repository';
...
jest.mock('./evaluation.repository');
...
describe('PermissionService Engine Rules', () => {
  ...
  describe('canPerformAction', () => {
    ...
    it('returns false if user not found', async () => {
      mockedRepo.getEvaluationData.mockImplementation(() => Promise.resolve(null));
      const result = await permissionService.canPerformAction(userId, 'reports:List' as IamAction);
      expect(result).toBe(false);
    });
